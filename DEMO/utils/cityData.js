const cityData = [
  {
    name: '中国',
    code: 'CN',
    provinces: [
      {
        name: '北京',
        code: '110000',
        cities: ['东城区', '西城区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴区', '怀柔区', '平谷区', '密云区', '延庆区']
      },
      {
        name: '上海',
        code: '310000',
        cities: ['黄浦区', '徐汇区', '长宁区', '静安区', '普陀区', '虹口区', '杨浦区', '闵行区', '宝山区', '嘉定区', '浦东新区', '金山区', '松江区', '青浦区', '奉贤区', '崇明区']
      },
      {
        name: '广东',
        code: '440000',
        cities: ['广州市', '韶关市', '深圳市', '珠海市', '汕头市', '佛山市', '江门市', '湛江市', '茂名市', '肇庆市', '惠州市', '梅州市', '汕尾市', '河源市', '阳江市', '清远市', '东莞市', '中山市', '潮州市', '揭阳市', '云浮市']
      },
      {
        name: '浙江',
        code: '330000',
        cities: ['杭州市', '宁波市', '温州市', '嘉兴市', '湖州市', '绍兴市', '金华市', '衢州市', '舟山市', '台州市', '丽水市']
      },
      {
        name: '江苏',
        code: '320000',
        cities: ['南京市', '无锡市', '徐州市', '常州市', '苏州市', '南通市', '连云港市', '淮安市', '盐城市', '扬州市', '镇江市', '泰州市', '宿迁市']
      },
      {
        name: '四川',
        code: '510000',
        cities: ['成都市', '自贡市', '攀枝花市', '泸州市', '德阳市', '绵阳市', '广元市', '遂宁市', '内江市', '乐山市', '南充市', '眉山市', '宜宾市', '广安市', '达州市', '雅安市', '巴中市', '资阳市', '阿坝藏族羌族自治州', '甘孜藏族自治州', '凉山彝族自治州']
      },
      {
        name: '湖北',
        code: '420000',
        cities: ['武汉市', '黄石市', '十堰市', '宜昌市', '襄阳市', '鄂州市', '荆门市', '孝感市', '荆州市', '黄冈市', '咸宁市', '随州市', '恩施土家族苗族自治州', '仙桃市', '潜江市', '天门市', '神农架林区']
      },
      {
        name: '湖南',
        code: '430000',
        cities: ['长沙市', '株洲市', '湘潭市', '衡阳市', '邵阳市', '岳阳市', '常德市', '张家界市', '益阳市', '郴州市', '永州市', '怀化市', '娄底市', '湘西土家族苗族自治州']
      },
      {
        name: '河南',
        code: '410000',
        cities: ['郑州市', '开封市', '洛阳市', '平顶山市', '安阳市', '鹤壁市', '新乡市', '焦作市', '濮阳市', '许昌市', '漯河市', '三门峡市', '南阳市', '商丘市', '信阳市', '周口市', '驻马店市', '济源市']
      },
      {
        name: '山东',
        code: '370000',
        cities: ['济南市', '青岛市', '淄博市', '枣庄市', '东营市', '烟台市', '潍坊市', '济宁市', '泰安市', '威海市', '日照市', '临沂市', '德州市', '聊城市', '滨州市', '菏泽市']
      },
      {
        name: '河北',
        code: '130000',
        cities: ['石家庄市', '唐山市', '秦皇岛市', '邯郸市', '邢台市', '保定市', '张家口市', '承德市', '沧州市', '廊坊市', '衡水市']
      },
      {
        name: '山西',
        code: '140000',
        cities: ['太原市', '大同市', '阳泉市', '长治市', '晋城市', '朔州市', '晋中市', '运城市', '忻州市', '临汾市', '吕梁市']
      },
      {
        name: '陕西',
        code: '610000',
        cities: ['西安市', '铜川市', '宝鸡市', '咸阳市', '渭南市', '延安市', '汉中市', '榆林市', '安康市', '商洛市']
      },
      {
        name: '辽宁',
        code: '210000',
        cities: ['沈阳市', '大连市', '鞍山市', '抚顺市', '本溪市', '丹东市', '锦州市', '营口市', '阜新市', '辽阳市', '盘锦市', '铁岭市', '朝阳市', '葫芦岛市']
      },
      {
        name: '吉林',
        code: '220000',
        cities: ['长春市', '吉林市', '四平市', '辽源市', '通化市', '白山市', '松原市', '白城市', '延边朝鲜族自治州']
      },
      {
        name: '黑龙江',
        code: '230000',
        cities: ['哈尔滨市', '齐齐哈尔市', '鸡西市', '鹤岗市', '双鸭山市', '大庆市', '伊春市', '佳木斯市', '七台河市', '牡丹江市', '黑河市', '绥化市', '大兴安岭地区']
      },
      {
        name: '安徽',
        code: '340000',
        cities: ['合肥市', '芜湖市', '蚌埠市', '淮南市', '马鞍山市', '淮北市', '铜陵市', '安庆市', '黄山市', '滁州市', '阜阳市', '宿州市', '六安市', '亳州市', '池州市', '宣城市']
      },
      {
        name: '福建',
        code: '350000',
        cities: ['福州市', '厦门市', '莆田市', '三明市', '泉州市', '漳州市', '南平市', '龙岩市', '宁德市']
      },
      {
        name: '江西',
        code: '360000',
        cities: ['南昌市', '景德镇市', '萍乡市', '九江市', '新余市', '鹰潭市', '赣州市', '吉安市', '宜春市', '抚州市', '上饶市']
      },
      {
        name: '云南',
        code: '530000',
        cities: ['昆明市', '曲靖市', '玉溪市', '保山市', '昭通市', '丽江市', '普洱市', '临沧市', '楚雄彝族自治州', '红河哈尼族彝族自治州', '文山壮族苗族自治州', '西双版纳傣族自治州', '大理白族自治州', '德宏傣族景颇族自治州', '怒江傈僳族自治州', '迪庆藏族自治州']
      },
      {
        name: '贵州',
        code: '520000',
        cities: ['贵阳市', '六盘水市', '遵义市', '安顺市', '毕节市', '铜仁市', '黔西南布依族苗族自治州', '黔东南苗族侗族自治州', '黔南布依族苗族自治州']
      },
      {
        name: '海南',
        code: '460000',
        cities: ['海口市', '三亚市', '三沙市', '儋州市', '五指山市', '琼海市', '文昌市', '万宁市', '东方市']
      },
      {
        name: '甘肃',
        code: '620000',
        cities: ['兰州市', '嘉峪关市', '金昌市', '白银市', '天水市', '武威市', '张掖市', '平凉市', '酒泉市', '庆阳市', '定西市', '陇南市', '临夏回族自治州', '甘南藏族自治州']
      },
      {
        name: '青海',
        code: '630000',
        cities: ['西宁市', '海东市', '海北藏族自治州', '黄南藏族自治州', '海南藏族自治州', '果洛藏族自治州', '玉树藏族自治州', '海西蒙古族藏族自治州']
      },
      {
        name: '内蒙古',
        code: '150000',
        cities: ['呼和浩特市', '包头市', '乌海市', '赤峰市', '通辽市', '鄂尔多斯市', '呼伦贝尔市', '巴彦淖尔市', '乌兰察布市', '兴安盟', '锡林郭勒盟', '阿拉善盟']
      },
      {
        name: '广西',
        code: '450000',
        cities: ['南宁市', '柳州市', '桂林市', '梧州市', '北海市', '防城港市', '钦州市', '贵港市', '玉林市', '百色市', '贺州市', '河池市', '来宾市', '崇左市']
      },
      {
        name: '西藏',
        code: '540000',
        cities: ['拉萨市', '日喀则市', '昌都市', '林芝市', '山南市', '那曲市', '阿里地区']
      },
      {
        name: '宁夏',
        code: '640000',
        cities: ['银川市', '石嘴山市', '吴忠市', '固原市', '中卫市']
      },
      {
        name: '新疆',
        code: '650000',
        cities: ['乌鲁木齐市', '克拉玛依市', '吐鲁番市', '哈密市', '昌吉回族自治州', '博尔塔拉蒙古自治州', '巴音郭楞蒙古自治州', '阿克苏地区', '克孜勒苏柯尔克孜自治州', '喀什地区', '和田地区', '伊犁哈萨克自治州', '塔城地区', '阿勒泰地区']
      },
      {
        name: '天津',
        code: '120000',
        cities: ['和平区', '河东区', '河西区', '南开区', '河北区', '红桥区', '东丽区', '西青区', '津南区', '北辰区', '武清区', '宝坻区', '滨海新区', '宁河区', '静海区', '蓟州区']
      },
      {
        name: '重庆',
        code: '500000',
        cities: ['万州区', '涪陵区', '渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '綦江区', '大足区', '渝北区', '巴南区', '黔江区', '长寿区', '江津区', '合川区', '永川区', '南川区', '璧山区', '铜梁区', '潼南区', '荣昌区', '开州区', '梁平区', '武隆区']
      },
      {
        name: '香港',
        code: '810000',
        cities: ['中西区', '湾仔区', '东区', '南区', '油尖旺区', '深水埗区', '九龙城区', '黄大仙区', '观塘区', '荃湾区', '屯门区', '元朗区', '北区', '大埔区', '沙田区', '西贡区', '葵青区', '离岛区']
      },
      {
        name: '澳门',
        code: '820000',
        cities: ['澳门半岛', '氹仔', '路环']
      },
      {
        name: '台湾',
        code: '710000',
        cities: ['台北市', '新北市', '桃园市', '台中市', '台南市', '高雄市', '基隆市', '新竹市', '嘉义市']
      }
    ]
  },
  {
    name: '泰国',
    code: 'TH',
    provinces: [
      {
        name: '曼谷',
        code: '10',
        cities: ['曼谷市区', '吞武里', '暖武里']
      },
      {
        name: '清迈',
        code: '50',
        cities: ['清迈市区', '清迈府']
      },
      {
        name: '普吉',
        code: '83',
        cities: ['普吉市区', '卡伦', '芭东']
      },
      {
        name: '芭提雅',
        code: '20',
        cities: ['芭提雅市区']
      },
      {
        name: '苏梅岛',
        code: '84',
        cities: ['苏梅岛']
      },
      {
        name: '清莱',
        code: '57',
        cities: ['清莱市区']
      },
      {
        name: '大城',
        code: '14',
        cities: ['大城市区']
      },
      {
        name: '华欣',
        code: '77',
        cities: ['华欣市区']
      }
    ]
  },
  {
    name: '马来西亚',
    code: 'MY',
    provinces: [
      {
        name: '吉隆坡',
        code: '14',
        cities: ['吉隆坡市区', '八打灵再也']
      },
      {
        name: '槟城',
        code: '07',
        cities: ['乔治市', '槟岛']
      },
      {
        name: '沙巴',
        code: '12',
        cities: ['亚庇', '仙本那']
      },
      {
        name: '砂拉越',
        code: '13',
        cities: ['古晋', '美里']
      },
      {
        name: '柔佛',
        code: '01',
        cities: ['新山']
      },
      {
        name: '马六甲',
        code: '04',
        cities: ['马六甲市区']
      },
      {
        name: '槟城',
        code: '07',
        cities: ['槟城市区']
      }
    ]
  },
  {
    name: '新加坡',
    code: 'SG',
    provinces: [
      {
        name: '新加坡',
        code: 'SG',
        cities: ['中部', '东部', '西部', '北部', '东北部']
      }
    ]
  },
  {
    name: '越南',
    code: 'VN',
    provinces: [
      {
        name: '河内',
        code: 'HN',
        cities: ['河内']
      },
      {
        name: '胡志明市',
        code: 'HCM',
        cities: ['胡志明市']
      },
      {
        name: '岘港',
        code: 'DN',
        cities: ['岘港']
      },
      {
        name: '下龙湾',
        code: 'QB',
        cities: ['下龙湾']
      },
      {
        name: '芽庄',
        code: 'KH',
        cities: ['芽庄']
      },
      {
        name: '会安',
        code: 'QA',
        cities: ['会安']
      },
      {
        name: '大叻',
        code: 'DL',
        cities: ['大叻']
      }
    ]
  },
  {
    name: '印度尼西亚',
    code: 'ID',
    provinces: [
      {
        name: '雅加达',
        code: 'JK',
        cities: ['雅加达']
      },
      {
        name: '巴厘岛',
        code: 'BA',
        cities: ['巴厘岛']
      },
      {
        name: '日惹',
        code: 'YO',
        cities: ['日惹']
      },
      {
        name: '龙目岛',
        code: 'NU',
        cities: ['龙目岛']
      },
      {
        name: '苏拉威西',
        code: 'SA',
        cities: ['望加锡']
      },
      {
        name: '苏门答腊',
        code: 'SU',
        cities: ['棉兰']
      }
    ]
  },
  {
    name: '韩国',
    code: 'KR',
    provinces: [
      {
        name: '首尔',
        code: '11',
        cities: ['首尔']
      },
      {
        name: '釜山',
        code: '26',
        cities: ['釜山']
      },
      {
        name: '济州',
        code: '49',
        cities: ['济州']
      },
      {
        name: '仁川',
        code: '28',
        cities: ['仁川']
      },
      {
        name: '大邱',
        code: '27',
        cities: ['大邱']
      },
      {
        name: '光州',
        code: '29',
        cities: ['光州']
      },
      {
        name: '大田',
        code: '30',
        cities: ['大田']
      },
      {
        name: '蔚山',
        code: '31',
        cities: ['蔚山']
      }
    ]
  },
  {
    name: '日本',
    code: 'JP',
    provinces: [
      {
        name: '东京都',
        code: '13',
        cities: ['新宿区', '涩谷区', '港区', '千代田区', '文京区']
      },
      {
        name: '大阪府',
        code: '27',
        cities: ['大阪市', '堺市', '东大阪市']
      },
      {
        name: '京都府',
        code: '26',
        cities: ['京都市', '宇治市']
      },
      {
        name: '北海道',
        code: '01',
        cities: ['札幌', '小樽']
      },
      {
        name: '福冈县',
        code: '40',
        cities: ['福冈', '北九州']
      },
      {
        name: '冲绳县',
        code: '47',
        cities: ['那霸', '宫古岛']
      },
      {
        name: '神奈川县',
        code: '14',
        cities: ['横滨', '镰仓']
      },
      {
        name: '爱知县',
        code: '23',
        cities: ['名古屋']
      },
      {
        name: '兵库县',
        code: '28',
        cities: ['神户']
      },
      {
        name: '广岛县',
        code: '34',
        cities: ['广岛']
      }
    ]
  },
  {
    name: '土耳其',
    code: 'TR',
    provinces: [
      {
        name: '伊斯坦布尔',
        code: '34',
        cities: ['伊斯坦布尔']
      },
      {
        name: '安卡拉',
        code: '06',
        cities: ['安卡拉']
      },
      {
        name: '安塔利亚',
        code: '07',
        cities: ['安塔利亚']
      },
      {
        name: '卡帕多奇亚',
        code: '50',
        cities: ['卡帕多奇亚']
      },
      {
        name: '伊兹密尔',
        code: '35',
        cities: ['伊兹密尔']
      },
      {
        name: '博德鲁姆',
        code: '48',
        cities: ['博德鲁姆']
      }
    ]
  },
  {
    name: '意大利',
    code: 'IT',
    provinces: [
      {
        name: '罗马',
        code: 'RM',
        cities: ['罗马']
      },
      {
        name: '米兰',
        code: 'MI',
        cities: ['米兰']
      },
      {
        name: '威尼斯',
        code: 'VE',
        cities: ['威尼斯']
      },
      {
        name: '佛罗伦萨',
        code: 'FI',
        cities: ['佛罗伦萨']
      },
      {
        name: '那不勒斯',
        code: 'NA',
        cities: ['那不勒斯']
      },
      {
        name: '都灵',
        code: 'TO',
        cities: ['都灵']
      },
      {
        name: '热那亚',
        code: 'GE',
        cities: ['热那亚']
      },
      {
        name: '博洛尼亚',
        code: 'BO',
        cities: ['博洛尼亚']
      }
    ]
  },
  {
    name: '法国',
    code: 'FR',
    provinces: [
      {
        name: '巴黎',
        code: '75',
        cities: ['巴黎']
      },
      {
        name: '里昂',
        code: '69',
        cities: ['里昂']
      },
      {
        name: '马赛',
        code: '13',
        cities: ['马赛']
      },
      {
        name: '尼斯',
        code: '06',
        cities: ['尼斯']
      },
      {
        name: '波尔多',
        code: '33',
        cities: ['波尔多']
      },
      {
        name: '里尔',
        code: '59',
        cities: ['里尔']
      },
      {
        name: '图卢兹',
        code: '31',
        cities: ['图卢兹']
      },
      {
        name: '南特',
        code: '44',
        cities: ['南特']
      },
      {
        name: '斯特拉斯堡',
        code: '67',
        cities: ['斯特拉斯堡']
      }
    ]
  },
  {
    name: '西班牙',
    code: 'ES',
    provinces: [
      {
        name: '马德里',
        code: 'MD',
        cities: ['马德里']
      },
      {
        name: '巴塞罗那',
        code: 'B',
        cities: ['巴塞罗那']
      },
      {
        name: '瓦伦西亚',
        code: 'V',
        cities: ['瓦伦西亚']
      },
      {
        name: '塞维利亚',
        code: 'SE',
        cities: ['塞维利亚']
      },
      {
        name: '毕尔巴鄂',
        code: 'BI',
        cities: ['毕尔巴鄂']
      },
      {
        name: '格拉纳达',
        code: 'GR',
        cities: ['格拉纳达']
      },
      {
        name: '马拉加',
        code: 'MA',
        cities: ['马拉加']
      },
      {
        name: '塞哥维亚',
        code: 'SG',
        cities: ['塞哥维亚']
      }
    ]
  },
  {
    name: '德国',
    code: 'DE',
    provinces: [
      {
        name: '柏林',
        code: 'BE',
        cities: ['柏林']
      },
      {
        name: '慕尼黑',
        code: 'BY',
        cities: ['慕尼黑']
      },
      {
        name: '汉堡',
        code: 'HH',
        cities: ['汉堡']
      },
      {
        name: '法兰克福',
        code: 'HE',
        cities: ['法兰克福']
      },
      {
        name: '科隆',
        code: 'NW',
        cities: ['科隆']
      },
      {
        name: '斯图加特',
        code: 'BW',
        cities: ['斯图加特']
      },
      {
        name: '杜塞尔多夫',
        code: 'NW',
        cities: ['杜塞尔多夫']
      },
      {
        name: '不来梅',
        code: 'HB',
        cities: ['不来梅']
      },
      {
        name: '德累斯顿',
        code: 'SN',
        cities: ['德累斯顿']
      }
    ]
  },
  {
    name: '瑞士',
    code: 'CH',
    provinces: [
      {
        name: '苏黎世',
        code: 'ZH',
        cities: ['苏黎世']
      },
      {
        name: '日内瓦',
        code: 'GE',
        cities: ['日内瓦']
      },
      {
        name: '因特拉肯',
        code: 'BE',
        cities: ['因特拉肯']
      },
      {
        name: '卢塞恩',
        code: 'LU',
        cities: ['卢塞恩']
      },
      {
        name: '巴塞尔',
        code: 'BS',
        cities: ['巴塞尔']
      },
      {
        name: '伯尔尼',
        code: 'BE',
        cities: ['伯尔尼']
      },
      {
        name: '洛桑',
        code: 'VD',
        cities: ['洛桑']
      }
    ]
  },
  {
    name: '英国',
    code: 'GB',
    provinces: [
      {
        name: '伦敦',
        code: 'LND',
        cities: ['伦敦']
      },
      {
        name: '曼彻斯特',
        code: 'MAN',
        cities: ['曼彻斯特']
      },
      {
        name: '爱丁堡',
        code: 'EDH',
        cities: ['爱丁堡']
      },
      {
        name: '利物浦',
        code: 'LIV',
        cities: ['利物浦']
      },
      {
        name: '伯明翰',
        code: 'BIR',
        cities: ['伯明翰']
      },
      {
        name: '格拉斯哥',
        code: 'GLA',
        cities: ['格拉斯哥']
      },
      {
        name: '牛津',
        code: 'OXF',
        cities: ['牛津']
      },
      {
        name: '剑桥',
        code: 'CAM',
        cities: ['剑桥']
      },
      {
        name: '巴斯',
        code: 'BAS',
        cities: ['巴斯']
      }
    ]
  },
  {
    name: '美国',
    code: 'US',
    provinces: [
      {
        name: '加利福尼亚州',
        code: 'CA',
        cities: ['洛杉矶', '旧金山', '圣地亚哥', '圣何塞', '萨克拉门托']
      },
      {
        name: '纽约州',
        code: 'NY',
        cities: ['纽约市', '布法罗', '罗切斯特', '奥尔巴尼']
      },
      {
        name: '德克萨斯州',
        code: 'TX',
        cities: ['休斯顿', '达拉斯', '奥斯汀', '圣安东尼奥']
      },
      {
        name: '佛罗里达州',
        code: 'FL',
        cities: ['迈阿密', '奥兰多', '坦帕', '杰克逊维尔']
      },
      {
        name: '华盛顿州',
        code: 'WA',
        cities: ['西雅图']
      },
      {
        name: '伊利诺伊州',
        code: 'IL',
        cities: ['芝加哥']
      },
      {
        name: '内华达州',
        code: 'NV',
        cities: ['拉斯维加斯']
      },
      {
        name: '夏威夷州',
        code: 'HI',
        cities: ['檀香山']
      },
      {
        name: '马萨诸塞州',
        code: 'MA',
        cities: ['波士顿']
      },
      {
        name: '宾夕法尼亚州',
        code: 'PA',
        cities: ['费城', '匹兹堡']
      },
      {
        name: '科罗拉多州',
        code: 'CO',
        cities: ['丹佛']
      },
      {
        name: '亚利桑那州',
        code: 'AZ',
        cities: ['凤凰城']
      },
      {
        name: '俄勒冈州',
        code: 'OR',
        cities: ['波特兰']
      },
      {
        name: '华盛顿特区',
        code: 'DC',
        cities: ['华盛顿特区']
      }
    ]
  },
  {
    name: '澳大利亚',
    code: 'AU',
    provinces: [
      {
        name: '新南威尔士州',
        code: 'NSW',
        cities: ['悉尼', '堪培拉']
      },
      {
        name: '维多利亚州',
        code: 'VIC',
        cities: ['墨尔本']
      },
      {
        name: '昆士兰州',
        code: 'QLD',
        cities: ['布里斯班', '黄金海岸']
      },
      {
        name: '西澳大利亚州',
        code: 'WA',
        cities: ['珀斯']
      },
      {
        name: '南澳大利亚州',
        code: 'SA',
        cities: ['阿德莱德']
      },
      {
        name: '塔斯马尼亚州',
        code: 'TAS',
        cities: ['霍巴特']
      },
      {
        name: '北领地',
        code: 'NT',
        cities: ['达尔文']
      }
    ]
  },
  {
    name: '新西兰',
    code: 'NZ',
    provinces: [
      {
        name: '奥克兰',
        code: 'AUK',
        cities: ['奥克兰']
      },
      {
        name: '惠灵顿',
        code: 'WGN',
        cities: ['惠灵顿']
      },
      {
        name: '皇后镇',
        code: 'OTA',
        cities: ['皇后镇']
      },
      {
        name: '基督城',
        code: 'CHC',
        cities: ['基督城']
      },
      {
        name: '罗托鲁瓦',
        code: 'ROT',
        cities: ['罗托鲁瓦']
      }
    ]
  },
  {
    name: '俄罗斯',
    code: 'RU',
    provinces: [
      {
        name: '莫斯科',
        code: 'MOW',
        cities: ['莫斯科']
      },
      {
        name: '圣彼得堡',
        code: 'LED',
        cities: ['圣彼得堡']
      },
      {
        name: '符拉迪沃斯托克',
        code: 'VVO',
        cities: ['符拉迪沃斯托克']
      },
      {
        name: '新西伯利亚',
        code: 'OVB',
        cities: ['新西伯利亚']
      },
      {
        name: '叶卡捷琳堡',
        code: 'SVX',
        cities: ['叶卡捷琳堡']
      },
      {
        name: '喀山',
        code: 'KZN',
        cities: ['喀山']
      },
      {
        name: '索契',
        code: 'AER',
        cities: ['索契']
      }
    ]
  },
  {
    name: '挪威',
    code: 'NO',
    provinces: [
      {
        name: '奥斯陆',
        code: '03',
        cities: ['奥斯陆']
      },
      {
        name: '卑尔根',
        code: '46',
        cities: ['卑尔根']
      },
      {
        name: '特罗姆瑟',
        code: 'TOS',
        cities: ['特罗姆瑟']
      },
      {
        name: '斯塔万格',
        code: 'SVG',
        cities: ['斯塔万格']
      },
      {
        name: '罗弗敦',
        code: 'LKN',
        cities: ['罗弗敦']
      }
    ]
  },
  {
    name: '马尔代夫',
    code: 'MV',
    provinces: [
      {
        name: '马累',
        code: 'MLE',
        cities: ['马累']
      },
      {
        name: '北马累环礁',
        code: 'NMA',
        cities: ['北马累环礁']
      },
      {
        name: '南马累环礁',
        code: 'SMA',
        cities: ['南马累环礁']
      },
      {
        name: '阿里环礁',
        code: 'AL',
        cities: ['阿里环礁']
      }
    ]
  },
  {
    name: '希腊',
    code: 'GR',
    provinces: [
      {
        name: '雅典',
        code: 'A',
        cities: ['雅典']
      },
      {
        name: '圣托里尼',
        code: '69',
        cities: ['圣托里尼']
      },
      {
        name: '米科诺斯',
        code: '81',
        cities: ['米科诺斯']
      },
      {
        name: '克里特岛',
        code: '94',
        cities: ['克里特岛']
      },
      {
        name: '罗德岛',
        code: '85',
        cities: ['罗德岛']
      },
      {
        name: '科孚岛',
        code: '21',
        cities: ['科孚岛']
      },
      {
        name: '纳克索斯',
        code: '51',
        cities: ['纳克索斯']
      }
    ]
  },
  {
    name: '葡萄牙',
    code: 'PT',
    provinces: [
      {
        name: '里斯本',
        code: '11',
        cities: ['里斯本']
      },
      {
        name: '波尔图',
        code: '13',
        cities: ['波尔图']
      },
      {
        name: '法鲁',
        code: '08',
        cities: ['法鲁']
      },
      {
        name: '马德拉',
        code: '31',
        cities: ['马德拉']
      },
      {
        name: '亚速尔群岛',
        code: '20',
        cities: ['亚速尔群岛']
      },
      {
        name: '科英布拉',
        code: '06',
        cities: ['科英布拉']
      }
    ]
  },
  {
    name: '荷兰',
    code: 'NL',
    provinces: [
      {
        name: '阿姆斯特丹',
        code: 'NH',
        cities: ['阿姆斯特丹']
      },
      {
        name: '鹿特丹',
        code: 'ZH',
        cities: ['鹿特丹']
      },
      {
        name: '海牙',
        code: 'ZH',
        cities: ['海牙']
      },
      {
        name: '乌得勒支',
        code: 'UT',
        cities: ['乌得勒支']
      },
      {
        name: '埃因霍温',
        code: 'NB',
        cities: ['埃因霍温']
      },
      {
        name: '马斯特里赫特',
        code: 'LI',
        cities: ['马斯特里赫特']
      }
    ]
  },
  {
    name: '奥地利',
    code: 'AT',
    provinces: [
      {
        name: '维也纳',
        code: '9',
        cities: ['维也纳']
      },
      {
        name: '萨尔茨堡',
        code: '5',
        cities: ['萨尔茨堡']
      },
      {
        name: '因斯布鲁克',
        code: '7',
        cities: ['因斯布鲁克']
      },
      {
        name: '格拉茨',
        code: '6',
        cities: ['格拉茨']
      },
      {
        name: '林茨',
        code: '4',
        cities: ['林茨']
      },
      {
        name: '哈尔施塔特',
        code: '4',
        cities: ['哈尔施塔特']
      }
    ]
  },
  {
    name: '捷克',
    code: 'CZ',
    provinces: [
      {
        name: '布拉格',
        code: '10',
        cities: ['布拉格']
      },
      {
        name: '布尔诺',
        code: '64',
        cities: ['布尔诺']
      },
      {
        name: '奥洛穆茨',
        code: '71',
        cities: ['奥洛穆茨']
      },
      {
        name: '比尔森',
        code: '32',
        cities: ['比尔森']
      },
      {
        name: '卡罗维发利',
        code: '51',
        cities: ['卡罗维发利']
      },
      {
        name: '克鲁姆洛夫',
        code: '79',
        cities: ['克鲁姆洛夫']
      }
    ]
  },
  {
    name: '阿联酋',
    code: 'AE',
    provinces: [
      {
        name: '迪拜',
        code: 'DU',
        cities: ['迪拜']
      },
      {
        name: '阿布扎比',
        code: 'AZ',
        cities: ['阿布扎比']
      },
      {
        name: '沙迦',
        code: 'SH',
        cities: ['沙迦']
      },
      {
        name: '阿治曼',
        code: 'AJ',
        cities: ['阿治曼']
      },
      {
        name: '拉斯海马',
        code: 'RK',
        cities: ['拉斯海马']
      }
    ]
  },
  {
    name: '埃及',
    code: 'EG',
    provinces: [
      {
        name: '开罗',
        code: 'C',
        cities: ['开罗']
      },
      {
        name: '亚历山大',
        code: 'ALX',
        cities: ['亚历山大']
      },
      {
        name: '卢克索',
        code: 'LX',
        cities: ['卢克索']
      },
      {
        name: '阿斯旺',
        code: 'ASN',
        cities: ['阿斯旺']
      },
      {
        name: '沙姆沙伊赫',
        code: 'SSH',
        cities: ['沙姆沙伊赫']
      },
      {
        name: '胡尔格达',
        code: 'HRG',
        cities: ['胡尔格达']
      }
    ]
  },
  {
    name: '南非',
    code: 'ZA',
    provinces: [
      {
        name: '开普敦',
        code: 'CPT',
        cities: ['开普敦']
      },
      {
        name: '约翰内斯堡',
        code: 'JNB',
        cities: ['约翰内斯堡']
      },
      {
        name: '德班',
        code: 'DUR',
        cities: ['德班']
      },
      {
        name: '伊丽莎白港',
        code: 'PLZ',
        cities: ['伊丽莎白港']
      },
      {
        name: '克鲁格国家公园',
        code: 'MP',
        cities: ['克鲁格国家公园']
      }
    ]
  },
  {
    name: '加拿大',
    code: 'CA',
    provinces: [
      {
        name: '安大略省',
        code: 'ON',
        cities: ['多伦多', '渥太华', '汉密尔顿']
      },
      {
        name: '魁北克省',
        code: 'QC',
        cities: ['蒙特利尔', '魁北克城']
      },
      {
        name: '不列颠哥伦比亚省',
        code: 'BC',
        cities: ['温哥华', '维多利亚']
      },
      {
        name: '阿尔伯塔省',
        code: 'AB',
        cities: ['卡尔加里', '埃德蒙顿']
      },
      {
        name: '曼尼托巴省',
        code: 'MB',
        cities: ['温尼伯']
      },
      {
        name: '萨斯喀彻温省',
        code: 'SK',
        cities: ['萨斯卡通', '里贾纳']
      },
      {
        name: '新斯科舍省',
        code: 'NS',
        cities: ['哈利法克斯']
      }
    ]
  },
  {
    name: '冰岛',
    code: 'IS',
    provinces: [
      {
        name: '雷克雅未克',
        code: '1',
        cities: ['雷克雅未克']
      },
      {
        name: '阿克雷里',
        code: '3',
        cities: ['阿克雷里']
      },
      {
        name: '维克',
        code: '8',
        cities: ['维克']
      },
      {
        name: '霍芬',
        code: '6',
        cities: ['霍芬']
      },
      {
        name: '蓝湖',
        code: '4',
        cities: ['蓝湖']
      }
    ]
  }
]

module.exports = cityData
